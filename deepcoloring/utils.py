...
def visualize(x_np, y_np, min_point=40, draw_text=True, cmap="Set1"):
    def softmax(x):
        """Compute stable softmax values for each set of scores in x."""
        x = x - numpy.max(x, axis=0, keepdims=True)  # 최대값 빼서 Overflow 방지
        exp_x = numpy.exp(x)
        return exp_x / (numpy.sum(exp_x, axis=0, keepdims=True) + 1e-8)  # 0 나누기 방지
    ...
    color_map = [(1., 1., 1., 1.)]
    import matplotlib
    colors = matplotlib.colormaps.get_cmap(cmap)  # 최신 버전 호환 코드
